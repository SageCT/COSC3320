...
using namespace std;
bool isSafe(int temp[], int n, int i,
            int start) { // if "i" column contains a chess piece(#)
...
int req_function(
    string arr[], int temp[], int n, int k, int start,
    int count) { // If you have already ploced k pieces, then return
  if (count == k) {
    return 1;
  ...
  for (int i = 0; i < n; i++) { // If # exists and position is safe
    if (arr[start][i] == '#' && isSafe(temp, n, i, start)) {
      temp[start] = i; // Call recursively to place pieces in the further rows
      totalWays =
          totalWays +
          req_function(arr, temp, n, k, start + 1,
                       count +
                           1); // Backtrack and remmove i from position[start]
      temp[start] = -1;
    }
